...
export function createApp() {
  ...
  app.get("/receipts/:slug", (req, res) => {
    const slug = String(req.params.slug);
    if (slug.length === 0) {
      return res.status(400).json({ error: "Invalid receipt slug." });
    }
    try {
      const row = getReceiptBySlug(slug);
      if (!row) {
        return res.status(404).json({ error: "Receipt not found." });
      }
      const line_items = listReceiptLineItems(row.id);
      const claim_messages = listReceiptClaimMessages(row.id);
      const receipt_claims = listReceiptClaimsForReceipt(row.id);
      res.json({
        receipt: {
          ...row,
  ...
  app.post("/receipts/:id/claims", async (req, res) => {
    ...
    const rawName =
      req.body?.name ?? req.body?.claimerName ?? req.body?.claimer_name;
    ...
    try {
      ...
      try {
        ...
        if (lineItems.length > 0) {
          const parsed = await parsePlainTextToReceiptClaims(text, lineItems, {
